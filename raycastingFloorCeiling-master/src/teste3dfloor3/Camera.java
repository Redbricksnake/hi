...
import static teste3dfloor3.pauseMenu.pauseMenu;
...
public class Camera {
    ...
    public void updateMovement(Map map) {
        ...
        if (Keyboard.keyDown[37]) { // left strafe
            strafe(map, -Time.delta * velocity);
        }
        else if (Keyboard.keyDown[39]) { // right strafe
            strafe(map, Time.delta * velocity);
        }
        ...
        if (Keyboard.keyDown[90]) { // z(left rotate)
            rotate(+Time.delta * 0.000000003);
        }
        else if (Keyboard.keyDown[88]) { //x (right rotate
            rotate(-Time.delta * 0.000000003);
        }
        if (Keyboard.keyDown[38]) {
            if (Keyboard.keyDown[16]) { // sprint
                forward(map, Time.delta * 0.00000010);
            }
        }
        if (Keyboard.keyDown[27]) { // Esc
